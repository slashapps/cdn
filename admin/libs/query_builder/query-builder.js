...
(function (root, factory) {
    ...
    QueryBuilder.OPERATORS = {
        equal: { type: 'equal', nb_inputs: 1, multiple: false, apply_to: ['string', 'number', 'datetime', 'date', 'time', 'boolean'] },
        not_equal: { type: 'not_equal', nb_inputs: 1, multiple: false, apply_to: ['string', 'number', 'datetime', 'date', 'time', 'boolean'] },
        in: { type: 'in', nb_inputs: 1, multiple: true, apply_to: ['string', /*'number', /*'datetime', 'date', 'time'*/] },
        not_in: { type: 'not_in', nb_inputs: 1, multiple: true, apply_to: ['string', /*'number', /*'datetime', 'date', 'time'*/] },
        less: { type: 'less', nb_inputs: 1, multiple: false, apply_to: ['number', 'datetime', 'date', 'time'] },
        less_or_equal: { type: 'less_or_equal', nb_inputs: 1, multiple: false, apply_to: ['number', 'datetime', 'date', 'time'] },
        greater: { type: 'greater', nb_inputs: 1, multiple: false, apply_to: ['number', 'datetime', 'date', 'time'] },
        greater_or_equal: { type: 'greater_or_equal', nb_inputs: 1, multiple: false, apply_to: ['number', 'datetime', 'date', 'time'] },
        between: { type: 'between', nb_inputs: 2, multiple: false, apply_to: [/*'number', 'datetime', 'date', 'time'*/] },
        not_between: { type: 'not_between', nb_inputs: 2, multiple: false, apply_to: [/*'number', 'datetime', 'date', 'time'*/] },
        ...
        is_null: { type: 'is_null', nb_inputs: 0, multiple: false, apply_to: ['string', 'number', 'datetime', 'date', 'time', 'boolean'] },
        is_not_null: { type: 'is_not_null', nb_inputs: 0, multiple: false, apply_to: ['string', 'number', 'datetime', 'date', 'time', 'boolean'] },
        asc: { type: 'asc', nb_inputs: 0, multiple: false, apply_to: ['order'] },
        desc: { type: 'desc', nb_inputs: 0, multiple: false, apply_to: ['order'] },
        //none:               { type: 'none',      nb_inputs: 0, multiple: false, apply_to: ['string', 'number', 'datetime', 'date', 'time', 'boolean'] }
    };
    ...
    QueryBuilder.prototype.bindEvents = function () {
        ...
        this.$el.on('change.queryBuilder', Selectors.rule_filter, function () {
            self.clearErrors(); // Added by Anton 26/02/19
        ...
        this.$el.on('click.queryBuilder', Selectors.delete_rule, function () {
            self.clearErrors(); // Added by Anton 26/02/19
            var $rule = $(this).closest(Selectors.rule_container);
            // self.deleteRule(self.getModel($rule)); // Changed by Anton 26/02/19
            setTimeout(function () { self.deleteRule(self.getModel($rule)); }, 1)
        });
        ...
        if (this.settings.allow_groups !== 0) {
            ...
            this.$el.on('click.queryBuilder', Selectors.delete_group, function () {
                self.clearErrors(); // Added by Anton 26/02/19
                var $group = $(this).closest(Selectors.group_container);
                // self.deleteGroup(self.getModel($group)); // Changed by Anton 26/02/19
                setTimeout(function () { self.deleteGroup(self.getModel($group)); }, 1);
            });
        }
    ...
    QueryBuilder.prototype.createRuleInput = function (rule) {
        ...
        $inputs.on('change keyup ' + (filter.input_event || ''), function () {
    ...
    QueryBuilder.prototype.updateError = function (node) {
        if (this.settings.display_errors) {
            if (node.error === null) {
                // node.$el.removeClass('has-error'); // Changed by Anton 26/02/19
                node.$el.removeClass('has-error').removeAttr('data-error_type')
                ...
                //     .find(QueryBuilder.selectors.error_container).eq(0)
                //     .attr('title', errorMessage) // Changed by Anton 26/02/19
                /*
                node.$el.addClass('has-error').attr('data-error_type', node.error[0]) // Changed 11/05/20
                    .find(QueryBuilder.selectors.error_container).eq(0)
        ...
        var out = (function parse(group) {
            ...
            group.each(function (rule) {
                ...
                var ruleData = {
                    ...
                    queryType: rule ? rule.queryType : null, // Added by Anton 26/02/19
                    // value: value, // Changed by Anton 26/02/19
                    value: (rule.queryType == 1 && ((rule.filter && rule.filter.inputType == 'checkbox') || ((rule.filter && rule.filter.inputType == 'select2') && (rule.operator.type == 'in' || rule.operator.type == 'not_in'))) && value && value.length > 0 && !$.isArray(value)) ? value.split(',') : value
                };

                if (rule.filter && rule.filter.inputType == 'select2') {
                    ruleData.specialcontain = (typeof rule.filter.specialcontain !== 'undefined' && rule.filter.specialcontain === true);
                }
        ...
        (function add(data, group) {
            ...
            data.rules.forEach(function (item) {
                ...
                if (item.rules !== undefined) {
                    ...
                    if (item.queryType) { // Added by Anton 26/02/19
                        model.queryType = item.queryType;
                    }
    ...
    QueryBuilder.prototype._validateValue = function (rule, value) {
        ...
        var isParameter = (rule.queryType == '2'); // Added by Anton 26/02/19
        var result = true;
        var tmp, tempValue;

        value = (!isParameter && (filter.inputType == 'checkbox' || (filter.inputType == 'select2' && (operator.type == 'in' || operator.type == 'not_in'))) && value && value.length > 0 && !$.isArray(value)) ? value.split(',') : value; // Added by Anton 26/02/19
            ...
            switch (filter.input) {
                ...
                default:
                    ...
                    for (var j = 0; j < tempValue.length; j++) {
                        ...
                        var typeCheck = isParameter ? 'parameter' : QueryBuilder.types[filter.type]; // Added by Anton 26/02/19

                        // switch (QueryBuilder.types[filter.type]) { // Changed by Anton 26/02/19
                        switch (typeCheck) {
                            case 'parameter': // Added by Anton 26/02/19
                                if (tempValue[j] === undefined || tempValue[j].length === 0) {
                                    if (!validation.allow_empty_value) {
                                        result = ['parameter_empty'];
                                    }
                                    break;
                                }
                                if (!/^[a-zA-Z][a-zA-Z_.\-~0-9]*$/igm.test(tempValue[j])) {
                                    result = ['parameter_not_valid'];
                                    break;
                                }
                                break;
                            ...
                            case 'datetime':
                                ...
                                /*
                                var pattern = false;
                                console.log(typeCheck, typeof validation.format !== 'undefined' && validation.format)
                                if(typeof validation.format !== 'undefined' && validation.format){
                                    pattern = new RegExp(validation.format);
                                } else if(typeCheck == 'datetime'){
                                    pattern = /^(?:(?:31(\/|-|\.)(?:0?[13578]|1[02]))\1|(?:(?:29|30)(\/|-|\.)(?:0?[13-9]|1[0-2])\2))(?:(?:1[6-9]|[2-9]\d)?\d{2})$|^(?:29(\/|-|\.)0?2\3(?:(?:(?:1[6-9]|[2-9]\d)?(?:0[48]|[2468][048]|[13579][26])|(?:(?:16|[2468][048]|[3579][26])00))))$|^(?:0?[1-9]|1\d|2[0-8])(\/|-|\.)(?:(?:0?[1-9])|(?:1[0-2]))\4(?:(?:1[6-9]|[2-9]\d)?\d{2}) ([0-9]{2})\:([0-9]{2})(?:\:([0-9]{2}))$/;
                                } else if(typeCheck == 'date'){
                                    pattern = /^(?:(?:31(\/|-|\.)(?:0?[13578]|1[02]))\1|(?:(?:29|30)(\/|-|\.)(?:0?[13-9]|1[0-2])\2))(?:(?:1[6-9]|[2-9]\d)?\d{2})$|^(?:29(\/|-|\.)0?2\3(?:(?:(?:1[6-9]|[2-9]\d)?(?:0[48]|[2468][048]|[13579][26])|(?:(?:16|[2468][048]|[3579][26])00))))$|^(?:0?[1-9]|1\d|2[0-8])(\/|-|\.)(?:(?:0?[1-9])|(?:1[0-2]))\4(?:(?:1[6-9]|[2-9]\d)?\d{2})$/;
                                } else if(typeCheck == 'time'){
                                    pattern = /^([0-9]{2})\:([0-9]{2})(?:\:)(?:([0-9]{2}))$/;
                                }
                                console.log(pattern, !pattern.test(tempValue[j]))
                                if(pattern && !pattern.test(tempValue[j])){
                                    result = [''+typeCheck+'_invalid'];
                                    console.log(result)
                                    break;
                                }
                                */
    ...
    // Changed by Anton 26/02/19: added class="group-cond" to input type="radio" (in QueryBuilder.templates.group)
        ...
        <div class="btn-group group-conditions"> \
          {{~ it.conditions: condition }} \
            <label class="btn btn-xs btn-primary"> \
              <input type="radio" name="{{= it.group_id }}_cond" value="{{= condition }}" class="group-cond"> {{= it.translate("conditions", condition) }} \
            </label> \
          {{~}} \
        </div> \
    ...
    // Changed by Anton 26/02/19: error-container move after rule-value-container (in QueryBuilder.templates.rule)
    ...
    QueryBuilder.defaults({
        mongoOperators: {
            ...
            asc: function (v) { return null; },
            desc: function (v) { return null; }
            // @formatter:on
        },
                    ...
                    if (rule.rules && rule.rules.length > 0) {
                        ...
                        if (ope.nb_inputs !== 0) {
                            ...
                            rule.value.forEach(function (v, i) {
                                ...
                                if (rule.type == 'integer' && !isNaN(v)) { // Added by Anton 26/02/19
                                    v = parseInt(v);
                                }
                                if (rule.type == 'double' && !isNaN(v)) { // Added by Anton 26/02/19
                                    v = parseFloat(v);
                                }
                                if (rule.type == 'boolean' && boolean_as_integer) {

                                    if (rule.queryType != '2' && rule.queryType != '3') {
                        ...
                        var getCustFunction = function (v) {
                            v = (v.indexOf('%') == 0) ? v.substr(1) : v;
                            v = (v.lastIndexOf('%') == v.length - 1) ? v.substr(0, v.length - 1) : v;
                            return v.replace(/{{field}}/g, field).replace(/\\/g, "");
                        };

                        var getCustParameter = function (v) {
                            v = v.replace(/\'/g, "");
                            v = (v.indexOf('%') == 0) ? '%{{' + v.substr(1) : '{{' + v;
                            v = (v.lastIndexOf('%') == v.length - 1) ? v.substr(0, v.length - 1) + '}}%' : v + '}}';
                            return '\'' + v.replace(/\'/g, "") + '\'';
                        ...
                        var getCustSpecialContain = function (v) {
                            v = v.replace(/\'/g, "");
                            v = (v.indexOf('%') == 0) ? v.substr(1) : v;
                            v = (v.lastIndexOf('%') == v.length - 1) ? v.substr(0, v.length - 1) : v;
                            var str = '(\',\'+{{field}}+\',\') ' + ((rule.operator == 'contains') ? 'LIKE' : 'NOT LIKE') + ' (\'%,{{value}},%\')';
                            return str.replace(/{{field}}/g, field).replace(/{{value}}/g, v);
                        };
                        ...
                        var ruleExpression = '';
                        if (rule.queryType == '3') {
                            ruleExpression = getCustFunction(value);
                        } else if (rule.queryType == '2') {
                            ruleExpression = field + ' ' + sqlFn(getCustParameter(value));
    ...
    QueryBuilder.regional['en'] = {
        ...
        // "add_rule": "Add rule", // Changed by Anton 26/02/19
        // "add_group": "Add group", // Changed by Anton 26/02/19
        // "delete_rule": "Delete", // Changed by Anton 26/02/19
        // "delete_group": "Delete", // Changed by Anton 26/02/19
        // "conditions": { // Changed by Anton 26/02/19
        //   "AND": "AND",
        //   "OR": "OR"
        // },
        "add_rule": "Add Rule",
        ...
        "errors": {
            ...
            // "datetime_invalid": "Invalid date format ({0})", // Changed by Anton 26/02/19
            "datetime_invalid": "Invalid datetime format",
            "date_invalid": "Invalid date format", // Added by Anton 26/02/19
            "time_invalid": "Invalid time format", // Added by Anton 26/02/19
            ...
            "parameter_empty": "Empty parameter", // Added by Anton 26/02/19
            "parameter_not_valid": "Invalid parameter name", // Added by Anton 26/02/19
        },
        "invert": "Invert",
        "NOT": "NOT"
    };
